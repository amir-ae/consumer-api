...
public interface IProductResource
{
    ...
    Task<ErrorOr<PaginatedList<ProductResponse>>> ByPageDetail(int? pageSize, int? pageIndex, bool? nextPage = null, 
        string? keyId = null, CancellationToken ct = default);
    Task<ErrorOr<List<ProductForListingResponse>>> All(CancellationToken ct = default);
    Task<ErrorOr<List<ProductResponse>>> AllDetail(CancellationToken ct = default);
    Task<ErrorOr<List<ProductResponse>>> DetailByCentreId(Guid centreId, CancellationToken ct = default);
