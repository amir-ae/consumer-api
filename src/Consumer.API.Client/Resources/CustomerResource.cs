...
public class CustomerResource : ICustomerResource
{
    ...
    public async Task<ErrorOr<List<CustomerForListingResponse>>> All(CancellationToken ct = default)
    {
        var uri = _client.BuildUri(Customers.All.Uri());
        return await _client.Get<List<CustomerForListingResponse>>(uri, ct);
    }
    
    public async Task<ErrorOr<List<CustomerResponse>>> AllDetail(CancellationToken ct = default)
    {
        var uri = _client.BuildUri(Customers.AllDetail.Uri());
        return await _client.Get<List<CustomerResponse>>(uri, ct);
    }
    ...
    public async Task<ErrorOr<CustomerOrderResponse>> PostOrder(PostCustomerOrderRequest request, CancellationToken ct = default)
    {
        var uri = _client.BuildUri(Customers.PostOrder.Uri());
        return await _client.PostAsJson<CustomerOrderResponse, PostCustomerOrderRequest>(uri, request, ct);
    }
