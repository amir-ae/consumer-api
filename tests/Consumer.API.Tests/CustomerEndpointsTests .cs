...
public class CustomerEndpointsTests : IntegrationTest
{
    ...
    [Theory]
    [LoadData("order")]
    public async Task add_should_create_new_customer_order(PostCustomerOrderRequest request)
    {
        var client = _factory.CreateClient();
       
        var response = await client.PostAsJsonAsync(Customers.PostOrder.Uri(), request);
       
        response.EnsureSuccessStatusCode();
        var content = await response.Content.ReadAsStringAsync();
        var result = JsonConvert.DeserializeObject<CustomerOrderResponse>(content);
        result.ShouldNotBeNull();
        result.Customer.FirstName.ShouldBe(request.Owner!.FirstName);
        result.Customer.LastName.ShouldBe(request.Owner.LastName);
        result.Product.ProductId.ShouldBe(request.Product.ProductId);
    }
